...
def process_image(image):
	...
	robopts = get_roboline_pts(image.shape, h_matrix, im_out)
...
def threshough(image):
	...
	slope = round(float((midy2-midy1)/(midx2-midx1)))
...
def get_roboline_pts(shape, transform, warpimage):
	midx = shape[1]/2
	endy = shape[0]+600
	...
	#cv2.line(warpimage,(midx,0),(midx,endy),(255,0,0),2)
	cv2.line(warpimage,(road_pt1x, road_pt1y),(road_pt2x, road_pt2y),(255,0,0),2)
	print "added roboline to image"
	cv2.imshow("Roboline Image", warpimage)
	cv2.waitKey(2000)
	cv2.destroyAllWindows()
